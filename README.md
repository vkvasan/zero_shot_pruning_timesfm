...
## 📜 Methodology
This branch implements a **pruning-time MoE controller** over multiple pruning experts (`Magnitude`, `Wanda`, `OBS`, `SNR`). The controller first selects a layer-local expert/variant using activation and Gram statistics, then applies a **forecast-aware greedy post-pass** to correct high-regret layer decisions on held-out windows. Diagnostic tooling is included for layerwise error attribution and pairwise interaction analysis.
