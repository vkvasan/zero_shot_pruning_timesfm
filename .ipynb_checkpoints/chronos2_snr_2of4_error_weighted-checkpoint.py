...
class GramStat:
    ...
    Gact: torch.Tensor          # unweighted gram [G,4,4] (or just diag)
    Cact: float
    m2: float                   # sum(x^2)
    m4: float                   # sum(x^4)
...
def prune_linear_fused_2of4(layer, Gs, Gn, Gact, gate, eps=1e-8):
    ...
    # optional: per-group normalize each expert so one can’t dominate purely by scale
    def norm4(s):
...
def collect_group_grams_signal_noise(
    ...
    def make_hook(layer_name: str):
        def pre_hook(_mod, inputs):
            ...
            # moments (on the same sampled xf/xg)
            vals = xf  # [N, Cg] float32 after sampling
            m2 = float((vals * vals).sum().item())
            m4 = float(((vals * vals) ** 2).sum().item())
            n_m = int(vals.numel())
            
            # temporal roughness only if original x was [B,T,C]
            dx2 = 0.0
            ndx = 0
...
def main():
    ...
    for name, layer in targets:
        ...
        S = season_strength_global
        T = trend_strength_global
        ...
        for name, layer in targets:
            ...
            gate, k_ex, r = gate_for_layer(st, S, T, eps=args.eps, temp=args.fusion_temp, floor=args.fusion_floor)[:3], \
                            gate_for_layer(st, S, T)[3], gate_for_layer(st, S, T)[4]
