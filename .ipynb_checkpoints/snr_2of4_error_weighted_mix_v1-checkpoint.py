#!/usr/bin/env python3
"""
snr_2of4_error_weighted_mix_v1.py
...
def main():
    ...
    ap.add_argument("--mix_alpha", type=float, default=1.0,
                    help="Blend uniform and error-weighted grams: G=(1-mix_alpha)*G_uniform + mix_alpha*G_error. 1=pure error-weighted, 0=uniform.")
    ...
    args = ap.parse_args()
    # Mix alpha clamp
    if args.mix_alpha < 0.0:
        args.mix_alpha = 0.0
    if args.mix_alpha > 1.0:
        args.mix_alpha = 1.0
    ...
    if args.score_mode == "sn_ratio2":
        ...
        # Mix uniform + error-weighted grams via per-window weights
        # w_mix = (1-mix_alpha)*1 + mix_alpha*w_error
        w_sig_sel = ((1.0 - args.mix_alpha) + args.mix_alpha * w_sel).astype(np.float32)
        w_noi_sel = None
        if args.mix_alpha != 1.0:
            print(f"[calib] mix grams: mix_alpha={args.mix_alpha:g} | w_mix(min/mean/max)=({w_sig_sel.min():.3f}/{w_sig_sel.mean():.3f}/{w_sig_sel.max():.3f})")
