#!/usr/bin/env python3
"""
snr_2of4_error_weighted_fair_multiseries_v1.py

Extends the "error-weighted fair" 2:4 SNR pruning script to support:
- Single-series mode via --col (same as before)
- Multi-series mode via --cols_regex (e.g., Electricity has MT_1..MT_320)
  * Calibrate/prune ONCE using a subset of columns (--calib_cols)
  * Evaluate baseline + pruned accuracy as the MACRO-average across ALL selected columns

Key points:
- Still "zero-shot" in the sense of no backprop / no optimizer steps.
- Uses TRAIN windows only for calibration (weights/grams). Test labels never used for calibration.
- Score modes: keep, ratio (legacy).

Example Electricity run:
python snr_2of4_error_weighted_fair_multiseries_v1.py \
  ...
  --calib_windows 512 --calib_select random \
  --batch 4 --calib_batch 4 \
  --include_regex "stacked_xf\\..*\\.(attn\\.(qkv_proj|out)|ff0|ff1)$" \
  --sample_rows_per_call 2048 --max_calls_per_layer 32 \
...
def load_series_matrix(csv_path: str, cols_regex: str) -> Tuple[np.ndarray, List[str]]:
    ...
    mat = df[cols].to_numpy(dtype=np.float32)  # [T, D]
    return mat, cols
...
def eval_one_series(tfm, series_1d: np.ndarray, train_end: int, context: int, horizon: int,
    ...
    if test_windows < 0:
        ...
        te = min(ts + test_windows, X_test_all.shape[0])
...
def main():
    ...
    ap.add_argument("--cols_regex", type=str, default="^MT_\\d+$",
                    help="Regex for multi-series columns (e.g., '^MT_\\d+$').")

    ap.add_argument("--train_end", type=int, default=49152)
    ap.add_argument("--train_frac", type=float, default=0.0,
                    help="If >0, use int(T*train_frac) as train_end.")
    ...
    ap.add_argument("--calib_cols", type=int, default=32,
                    help="In multi-series mode, #columns used to build calibration pool.")
    ...
    ap.add_argument("--calib_windows", type=int, default=512,
                    help="Per-series calib pool size; multi-series pool concatenates calib_cols*calib_windows.")
    ap.add_argument("--calib_select", type=str, default="random", choices=["first", "random", "topk"])

    ap.add_argument("--test_windows", type=int, default=-1)
    ...
    if single_mode:
        ...
        use_c = min(D, int(args.calib_cols))
        if args.calib_cols_select == "first":
            calib_js = np.arange(use_c, dtype=np.int64)
        else:
            calib_js = rng.choice(D, size=use_c, replace=False).astype(np.int64)
        ...
        series_list = list(zip(colnames, [mat[:, j] for j in range(D)]))
        print(f"[mode] multi-series cols={len(colnames)} regex={args.cols_regex} calib_cols={use_c}")
    ...
    print(f"[calib] eval_batch={args.batch} calib_batch={args.calib_batch} "
    ...
    if args.measure_time:
        print(f"[baseline] MACRO_AVG over {len(mses_b)}/{len(series_list)} series: "
              f"MSE={mse_b:.6f} MAE={mae_b:.6f} | avg_batch_sec={float(np.mean(tb)) if tb else 0.0:.4f}")
    else:
        print(f"[baseline] MACRO_AVG over {len(mses_b)}/{len(series_list)} series: MSE={mse_b:.6f} MAE={mae_b:.6f}")
    ...
    if need_errors:
        ...
        errors, _err_ratio, weights = compute_errors_and_weights(
    ...
    gram_stats = collect_group_grams(
        tfm_model=tfm,
        targets=targets,
        X_sel=X_sel,
        w_sel=w_sel,
    ...
    if args.measure_time:
        print(f"[snr-2of4-refit] MACRO_AVG over {len(mses_p)}/{len(series_list)} series: "
              f"MSE={mse_p:.6f} MAE={mae_p:.6f} | avg_batch_sec={float(np.mean(tp)) if tp else 0.0:.4f}")
    else:
        print(f"[snr-2of4-refit] MACRO_AVG over {len(mses_p)}/{len(series_list)} series: MSE={mse_p:.6f} MAE={mae_p:.6f}")
