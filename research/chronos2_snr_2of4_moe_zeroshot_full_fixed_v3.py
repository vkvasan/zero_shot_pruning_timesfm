...
def timed_eval(pipe, pipe_kind: str, X: np.ndarray, Y: np.ndarray, horizon: int, batch: int, num_samples: int) -> Tuple[Dict[str, float], float]:
    if X is None or len(X) == 0:
        raise ValueError(
            "No test windows were generated (X_test is empty). This usually means train_end is too large for the dataset length, "
            "or context+horizon is too long. Reduce --train_end and/or --context/--horizon."
        )
...
class GramStat:
    ...
    s4: float                   # sum(x^4)
    n: int                      # count(x)
    dx2: float                  # sum((x_t-x_{t-1})^2)
    ndx: int                    # count(diff)

    # Extra robust-tail and stability stats for MoE gating
    tail_buf: np.ndarray = field(default_factory=lambda: np.empty((0,), dtype=np.float32))
    tail_fill: int = 0
    tail_seen: int = 0
    diag_mean_sum: float = 0.0
    diag_mean_sq_sum: float = 0.0
    diag_calls: int = 0
...
# ---------------------------------------------------------------------------
# Zero-shot (label-free) predictability weights + robust MoE gating utilities
# ---------------------------------------------------------------------------

def compute_zeroshot_predictability_weights(
    X_pool: np.ndarray,
    *,
    sn_gamma: float = 1.0,
    zs_beta_trend: float = 0.7,
    zs_beta_noise: float = 1.0,
    zs_k: float = 2.0,
    eps: float = 1e-8,
):
    '''
    Produce two per-window weights (signal vs noise) using ONLY the input context X (no labels).

    Intuition:
      - signal weight increases when a window is more predictable (strong periodic peak and/or strong trend, low jitter)
      - noise weight increases when it is less predictable (more high-frequency jitter)

    Returns: (w_sig, w_noi, stats_dict)
    '''
    X = np.asarray(X_pool, dtype=np.float64)
    N, L = X.shape
    if N == 0:
        raise ValueError("X_pool is empty; cannot compute zero-shot weights.")
    Xc = X - X.mean(axis=1, keepdims=True)

    # scale
    var = Xc.var(axis=1) + eps
    std = np.sqrt(var)

    # trend strength (dimensionless)
    t = np.arange(L, dtype=np.float64)
    t = t - t.mean()
    den = np.sum(t * t) + eps
    slope = (Xc @ t) / den  # (N,)
    trend = (np.abs(slope) * L) / (std + eps)

    # noise/jitter strength: var(diff) / var(level)
    dx = np.diff(Xc, axis=1)
    noise = (dx.var(axis=1) + eps) / var

    # seasonality peakiness: max power / total power (excluding DC)
    ...
    p = season + zs_beta_trend * trend - zs_beta_noise * noise

    # robust z + sigmoid
    med = np.median(p)
    mad = np.median(np.abs(p - med)) + eps
    z = (p - med) / (1.4826 * mad)
    w_sig = 1.0 / (1.0 + np.exp(-zs_k * z))
    w_noi = 1.0 - w_sig

    # shape + normalize to mean=1
    w_sig = np.power(w_sig + eps, sn_gamma)
    w_noi = np.power(w_noi + eps, sn_gamma)
    w_sig = w_sig / (w_sig.mean() + eps)
    w_noi = w_noi / (w_noi.mean() + eps)

    stats = {
        "p_median": float(med),
        "p_mad": float(mad),
        "season_mean": float(np.mean(season)),
        "trend_mean": float(np.mean(trend)),
        "noise_mean": float(np.mean(noise)),
        "w_sig_min": float(np.min(w_sig)),
        "w_sig_max": float(np.max(w_sig)),
        "w_noi_min": float(np.min(w_noi)),
        "w_noi_max": float(np.max(w_noi)),
    }
    return w_sig.astype(np.float32), w_noi.astype(np.float32), stats
...
def _reservoir_update(st, new_vals: np.ndarray, rng: np.random.Generator, reservoir_size: int):
    # st has: tail_buf (np.ndarray), tail_fill (int), tail_seen (int)
    if reservoir_size <= 0:
        return
    if st.tail_buf.size != reservoir_size:
        st.tail_buf = np.zeros((reservoir_size,), dtype=np.float32)
        st.tail_fill = 0
        st.tail_seen = 0

    buf = st.tail_buf
    fill = int(st.tail_fill)
    seen = int(st.tail_seen)

    vals = np.asarray(new_vals, dtype=np.float32).ravel()
    if vals.size == 0:
        return

    # First fill remaining slots
    if fill < reservoir_size:
        n_add = min(reservoir_size - fill, vals.size)
        buf[fill:fill + n_add] = vals[:n_add]
        fill += n_add
        seen += n_add
        vals = vals[n_add:]
...
def diag_from_gram(G, C: float, eps: float = 1e-8) -> np.ndarray:
    """Return diagonal of grouped-4 gram normalized by its effective count.

    G: [G,4,4] tensor/cpu tensor
    C: effective weighted count (sum of weights)
    Returns: np.ndarray [G,4] float64
    """
    if G is None:
        return np.zeros((0, 4), dtype=np.float64)
    denom = max(float(C), eps)
    if isinstance(G, torch.Tensor):
        d = torch.diagonal(G, dim1=-2, dim2=-1).detach().float().cpu().numpy()
    else:
        d = np.diagonal(np.asarray(G), axis1=-2, axis2=-1).astype(np.float64)
    return (d / denom).astype(np.float64)

def compute_layer_features_for_gating(st: "GramStat", eps: float = 1e-8):
    # SNR separation: mean log(ds/dn)
    ds = diag_from_gram(st.Gsig, st.Csig).astype(np.float64)
    dn = diag_from_gram(st.Gnoi, st.Cnoi).astype(np.float64)
    sep = float(np.mean(np.log((ds + eps) / (dn + eps))))

    # Tail metric: log(p99/p50) from reservoir |x|
    tail = 0.0
    if getattr(st, "tail_fill", 0) >= 32 and getattr(st, "tail_buf", np.empty((0,), np.float32)).size > 0:
        buf = st.tail_buf[: st.tail_fill].astype(np.float64)
        p50 = np.percentile(buf, 50)
        p99 = np.percentile(buf, 99)
        tail = float(np.log((p99 + eps) / (p50 + eps)))

    # Roughness: E[(dx)^2] / Var(x)
    n = max(int(st.n), 1)
    mu = float(st.s1 / n)
    m2 = float(st.s2 / n)
    var = max(m2 - mu * mu, 0.0) + eps
    rough = float((st.dx2 / max(st.ndx, 1.0)) / var)

    # Instability: CV of per-call mean diag(H) (uniform grams)
    instab = 0.0
    if getattr(st, "diag_calls", 0) >= 2:
        c = float(st.diag_calls)
        m = float(st.diag_mean_sum) / c
        v = max(float(st.diag_mean_sq_sum) / c - m * m, 0.0)
        instab = float(math.sqrt(v) / (abs(m) + eps))

    return {"snr_sep": sep, "tail": tail, "rough": rough, "instab": instab}


def compute_fused_gate_zscore(
    layer_features: dict,
    *,
    ...
    '''
    Robust MoE gate:
      - z-score features across layers (median/MAD)
      - build logits; softmax; apply optional floor.
    '''
    names = list(layer_features.keys())
    snr_sep = np.array([layer_features[n]["snr_sep"] for n in names], dtype=np.float64)
    tail = np.array([layer_features[n]["tail"] for n in names], dtype=np.float64)
    rough = np.array([layer_features[n]["rough"] for n in names], dtype=np.float64)
    instab = np.array([layer_features[n]["instab"] for n in names], dtype=np.float64)

    z_sep = robust_zscore(snr_sep)
    z_tail = robust_zscore(tail)
    z_rough = robust_zscore(rough)
    z_inst = robust_zscore(instab)

    gates = {}
    extras = {}

    # global priors: more season/trend -> slightly more SNR-ish
    prior_snr = 0.6 * season_strength + 0.3 * trend_strength
    prior_wanda = 0.2 * season_strength
    prior_mag = 0.0

    for i, n in enumerate(names):
        # logits (kept intentionally modest so softmax doesn't saturate)
        logit_snr = 0.9 * z_sep[i] - 0.6 * z_tail[i] - 0.6 * z_inst[i] - 0.2 * z_rough[i] + prior_snr
        logit_wan = 0.8 * z_tail[i] + 0.2 * z_rough[i] - 0.5 * z_sep[i] + prior_wanda
        logit_mag = 0.9 * z_inst[i] - 0.2 * z_sep[i] + prior_mag

        a, b, c = softmax3(logit_snr, logit_wan, logit_mag, temp=temp)
        if floor > 0:
            a = max(a, floor)
            b = max(b, floor)
            c = max(c, floor)
        ...
        gates[n] = (a, b, c)
        extras[n] = {
            "snr_sep": float(snr_sep[i]),
            "tail": float(tail[i]),
            "rough": float(rough[i]),
            "instab": float(instab[i]),
            "logits": (float(logit_snr), float(logit_wan), float(logit_mag)),
        }
...
def compute_season_trend_strength(X_sel: np.ndarray, ma_win: int, eps: float = 1e-8) -> Tuple[float, float, int]:
    ...
    Strengths are ratios computed from a simple variance decomposition:
      trend  = moving average ("same" length as input)
      season = x - trend
      strength = var(component) / var(x)

    Notes:
    - We deliberately keep this cheap + robust (no STL).
    - Uses reflect padding so the moving average has the same length as X.
    """
    ...
    W = int(ma_win)
    W = max(3, min(W, L))          # allow W==L
    if W % 2 == 0:
        W += 1
    W = min(W, L)                  # re-clip after making it odd
    pad = W // 2

    # Reflect padding + cumsum moving average with SAME output length L.
    Xpad = np.pad(X, ((0, 0), (pad, pad)), mode="reflect")  # [N, L+2*pad]
    c = np.cumsum(Xpad, axis=1, dtype=np.float64)           # [N, L+2*pad]
    c0 = np.concatenate([np.zeros((c.shape[0], 1), dtype=np.float64), c], axis=1)  # [N, L+2*pad+1]

    # window sum for positions i=0..L-1 is c0[i+W]-c0[i]
    ma = (c0[:, W:W + L] - c0[:, :L]) / float(W)            # [N, L]

    X64 = X.astype(np.float64)
    season = X64 - ma
    varx = np.var(X64, axis=1) + eps
...
def gate_for_layer(st: GramStat,
    ...
    Kurtosis here is **CENTERED excess kurtosis**, computed from accumulated raw sums (s1..s4)
    so it remains meaningful even when the activation mean is not ~0.
    """
    n = max(int(st.n), 1)

    # Raw (non-centered) moments
    m1 = (st.s1 / n)
    m2 = (st.s2 / n)
    m3 = (st.s3 / n)
    m4 = (st.s4 / n)

    # Centered variance and 4th central moment
    var = max(m2 - m1 * m1, 0.0) + eps
    mu4 = m4 - 4.0 * m1 * m3 + 6.0 * (m1 ** 2) * m2 - 3.0 * (m1 ** 4)

    k_excess = (mu4 / (var * var)) - 3.0

    # roughness in units of variance; only meaningful when we saw [B,T,C]
    if int(st.ndx) > 0:
        r = (st.dx2 / max(int(st.ndx), 1)) / var
    else:
        r = 0.0

    relu_k = max(0.0, k_excess)

    # Stabilize: huge kurtosis values should not explode logits
    tail = np.log1p(relu_k)  # 0..inf, but grows slowly

    # logits: push SNR when season/trend strong and activations are not too spiky
    # (WANDA tends to be robust for spiky/heavy-tailed activations)
    logit_snr   = +1.2 * season_strength + 0.6 * trend_strength - 0.8 * tail - 0.4 * r
    logit_wanda = +0.3 + 1.0 * tail + 0.3 * r + 0.3 * season_strength   # slight prior toward WANDA
    logit_mag   = -1.0                                                  # discourage MAG unless needed

    t = max(float(temp), 1e-6)
    w = softmax3([logit_snr / t, logit_wanda / t, logit_mag / t])
...
def collect_group_grams_signal_noise(
    ...
    num_samples: int,
    seed: int = 0,
    tail_reservoir: int = 4096,
    tail_per_call: int = 256,
) -> Dict[str, GramStat]:
    ...
    def make_hook(layer_name: str):
        def pre_hook(_mod, inputs):
            ...
            # --- Activation moments for gating (CENTERED kurtosis proxy) ---
            # Accumulate raw sums so we can compute centered kurtosis robustly across many hook calls.
            vals64 = xf.double()  # [N, Cg]
            s1 = float(vals64.sum().item())
            s2 = float((vals64 * vals64).sum().item())
            s3 = float((vals64 * vals64 * vals64).sum().item())
            s4 = float(((vals64 * vals64) ** 2).sum().item())
            n_m = int(vals64.numel())
            ...
            # --- MoE gating extras: tail reservoir + gram stability ---
            try:
                diagm = torch.diagonal(Gact_batch, dim1=-2, dim2=-1).float().mean().item()
                st.diag_mean_sum += float(diagm)
                st.diag_mean_sq_sum += float(diagm) * float(diagm)
                st.diag_calls += 1
            except Exception:
                pass

            if tail_per_call > 0 and tail_reservoir > 0:
                abs_flat = xf.abs().flatten()
                if abs_flat.numel() > 0:
                    if abs_flat.numel() > tail_per_call:
                        ridx = torch.randint(0, abs_flat.numel(), (tail_per_call,), device=abs_flat.device)
                        samp = abs_flat.index_select(0, ridx)
                    else:
                        samp = abs_flat
                    _reservoir_update(st, samp.detach().float().cpu().numpy(), rng, tail_reservoir)
...
def parse_args():
    ...
    ap.add_argument("--weight_mode", choices=["error", "zeroshot"], default="error",
                    help="How to build per-window signal/noise weights for grams. 'error' uses Y_calib; 'zeroshot' uses only X.")
    ap.add_argument("--gate_mode", choices=["zscore", "legacy"], default="zscore",
                    help="Fused MoE gating rule. 'zscore' is robust across-layer gating; 'legacy' keeps the older heuristic.")
    ap.add_argument("--force_expert", choices=["none", "snr", "wanda", "mag"], default="none",
                    help="Force all layers to use a single expert (debug / ablation).")
    ap.add_argument("--tail_reservoir", type=int, default=4096,
                    help="Reservoir size (per layer) for the robust tail metric used in fused gating.")
    ap.add_argument("--tail_per_call", type=int, default=256,
                    help="How many |activation| samples to add per gram call (per layer) for the tail reservoir.")
    ap.add_argument("--zs_beta_trend", type=float, default=0.7, help="Zero-shot weight: trend coefficient.")
    ap.add_argument("--zs_beta_noise", type=float, default=1.0, help="Zero-shot weight: noise/jitter coefficient.")
    ap.add_argument("--zs_k", type=float, default=2.0, help="Zero-shot weight: sigmoid steepness.")
    ap.add_argument("--fusion_temp", type=float, default=1.0, help="Softmax temperature for fused gating (lower => sharper).")
    ap.add_argument("--fusion_floor", type=float, default=0.0, help="Minimum probability per expert in fused gating.")
...
def main():
    ...
    for ci, col in enumerate(cols):
        ...
        # window weights on pool (error-weighted or zero-shot)
        if args.weight_mode == "error":
            print(f"[calib] computing errors/weights on pool: power={args.error_power} ...")
            errs = compute_errors_and_weights(pipe, pipe_kind, X_pool, Y_pool, args.horizon, args.batch, args.num_samples)
            # stronger errors -> noise; smaller errors -> signal
            err_ratio = (errs + args.weight_eps) / (np.median(errs) + args.weight_eps)
            w_sig = np.power(err_ratio, -args.sn_gamma).astype(np.float32)
            w_noi = np.power(err_ratio, +args.sn_gamma).astype(np.float32)
            w_sig = w_sig / (w_sig.mean() + 1e-8)
            w_noi = w_noi / (w_noi.mean() + 1e-8)
            ...
            print("[calib] computing ZERO-SHOT predictability weights on pool (no labels) ...")
            w_sig, w_noi, zs = compute_zeroshot_predictability_weights(
                X_pool,
                sn_gamma=args.sn_gamma,
                zs_beta_trend=args.zs_beta_trend,
                zs_beta_noise=args.zs_beta_noise,
                zs_k=args.zs_k,
                eps=1e-8,
            )
            print(f"[calib] zeroshot_stats: season_mean={zs['season_mean']:.3f} trend_mean={zs['trend_mean']:.3f} noise_mean={zs['noise_mean']:.3f} | "
                  f"w_sig[min,max]=[{zs['w_sig_min']:.3f},{zs['w_sig_max']:.3f}] w_noi[min,max]=[{zs['w_noi_min']:.3f},{zs['w_noi_max']:.3f}]")
        ...
        grams = collect_group_grams_signal_noise(
            ...
            tail_per_call=args.tail_per_call,
        )
        print(f"[calib] collected grams for {len(grams)}/{len(targets)} layers")

        # Global (per-column) season/trend proxy for gating (zero-shot; uses inputs only).
        # Needed for gate_mode=zscore and heuristic gating when score_mode=fused.
        season_strength = 0.0
        trend_strength = 0.0
        if args.score_mode == "fused":
            season_strength, trend_strength, usedW = compute_season_trend_strength(X_sel, args.decomp_ma, eps=args.eps)
            print(f"[gate] input season_strength={season_strength:.3f} trend_strength={trend_strength:.3f} (ma={usedW})")

        # prune
        if args.score_mode != "keep":
            if args.score_mode == "fused":
                print(
                    f"[prune] FUSED 2:4: MoE(SNR,WANDA,MAG) | temp={args.fusion_temp} floor={args.fusion_floor} "
                    f"gate_mode={args.gate_mode} force={args.force_expert}"
                )

                gates = None
                extras = {}

                if args.force_expert != "none":
                    if args.force_expert == "snr":
                        forced_gate = (1.0, 0.0, 0.0)
                    elif args.force_expert == "wanda":
                        forced_gate = (0.0, 1.0, 0.0)
                    elif args.force_expert == "mag":
                        forced_gate = (0.0, 0.0, 1.0)
                    else:
                        forced_gate = (1.0, 0.0, 0.0)
                    gates = {n: forced_gate for n in grams.keys()}
                elif args.gate_mode == "zscore":
                    layer_feats = {n: compute_layer_features_for_gating(st, eps=1e-8) for n, st in grams.items()}
                    gates, extras = compute_fused_gate_zscore(
                        layer_feats,
                ...
                for name, st in grams.items():
                    if gates is not None:
                        a, b, c = gates[name]
                    else:
                        a, b, c, _, _ = gate_for_layer(
                            st,
                            season_strength=season_strength,
                            trend_strength=trend_strength,
                            temp=args.fusion_temp,
                    ...
                    # print extra diagnostics (including robust tail / sep / instab)
                    if args.print_gate:
                        # kurtosis proxy (log1p of excess kurtosis), always computed the same way for readability
                        n = max(int(st.n), 1)
                        mu = float(st.s1) / n
                        m2 = float(st.s2) / n
                        m3 = float(st.s3) / n
                        m4 = float(st.s4) / n
                        var = max(m2 - mu * mu, 0.0) + 1e-8
                        m4c = max(m4 - 4 * mu * m3 + 6 * (mu ** 2) * m2 - 3 * (mu ** 4), 0.0)
                        k = m4c / (var * var + 1e-8)
                        kurt = float(math.log1p(max(k - 3.0, 0.0)))

                        # roughness
                        rough = float((st.dx2 / max(float(st.ndx), 1.0)) / var)

                        # robust tail / sep / instab if available
                        sep = float(np.mean(np.log((diag_from_gram(st.Gsig, st.Csig) + 1e-8) /
                                                   (diag_from_gram(st.Gnoi, st.Cnoi) + 1e-8))))
                        tail = 0.0
                        if getattr(st, "tail_fill", 0) >= 32 and getattr(st, "tail_buf", np.empty((0,), np.float32)).size > 0:
                            buf = st.tail_buf[: st.tail_fill].astype(np.float64)
                            p50 = np.percentile(buf, 50)
                            p99 = np.percentile(buf, 99)
                            tail = float(np.log((p99 + 1e-8) / (p50 + 1e-8)))
                        instab = 0.0
                        if getattr(st, "diag_calls", 0) >= 2:
                            ccall = float(st.diag_calls)
                            mm = float(st.diag_mean_sum) / ccall
                            vv = max(float(st.diag_mean_sq_sum) / ccall - mm * mm, 0.0)
                            instab = float(math.sqrt(vv) / (abs(mm) + 1e-8))

                        logits = extras.get(name, {}).get("logits", None)
                        if logits is not None:
                            lg = f" logits=({logits[0]:+.2f},{logits[1]:+.2f},{logits[2]:+.2f})"
                        else:
                            lg = ""

                        print(
                            f"[gate] {name}: snr={a:.2f} wanda={b:.2f} mag={c:.2f} | "
                            f"sep={sep:+.3f} tail={tail:+.3f} instab={instab:.3f} rough={rough:.2f} kurt={kurt:.2f}{lg}"
                        )

                    prune_linear_fused_2of4(
                        model, name, st,
                        group=4,
                        gate=(a, b, c),
